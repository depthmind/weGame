...
function onSelSess(sess_type, to_id) {
    if (selToID != null) {
    	this.setCookie("identifier", to_id);
    	$("#imId").val(readCookie("identifier"));
	    $("#imId1").val(readCookie("identifier"));
	    $("#imId2").val(readCookie("identifier"));
        //将之前选中用户的样式置为未选中样式
    ...
    if (selSessDiv) {
        selSessDiv.className = "sessinfo-sel"; //设置当前选中用户的样式为选中样式
					 $.ajax({
						  type : "get",
						  async : false, //同步请求
						  url:'./queryOldCustomer.do?imId='+newSelToID,
						  //data : data,
						  timeout:1000,
						  success:function(result){
							  if(result != "") {
								  ...
								  var d= res.data["0"];
								//要刷新的div
								  $('input[name=chineseName]').val(d.chineseName);
								  $('input[name=englishName]').val(d.englishName);
								  $('input[name=mobile]').val(d.mobilephone);
								  $('input[name=wechat]').val(d.wechat);
								  $('input[name=qq]').val(d.qq);
								  $('input[name=email]').val(d.email);
								 /* $('input[name=operator]').val(d.exclusiveUser);*/
								  ...
								  $('input[name=preferLanguage]').val("");
								    $(".language-select").select2({
								    	placeholder: '选择一个沟通语言',
								     	data: language
								    });
								    $('input[name=tailormade]').val("");
							      ...
							      $('input[name=during]').val("0");
							      $('input[name=budget]').val("0");
							      $('input[name=promote_code]').val("");
							      $('#startmonth').val("");
