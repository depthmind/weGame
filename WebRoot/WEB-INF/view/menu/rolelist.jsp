	...
	<section>
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				<h2>
					<i class="fa fa-key"></i> 权限管理 <span>角色列表</span>
				</h2>
...
<div class="modal fade" id="noDelModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static">
  <div class="modal-dialog modal-sm">
	...
	<script type="text/javascript">
		...
		function edit(id) {
			window.parent.location = "${rootPath}menu/roleedit.html?id="+id;
		}
		
		function del(id) {
			$("#confirmDelModal .hiddenId").val("");
			$("#confirmDelModal .hiddenId").val(id);
			$.post("${rootPath}menu/checkdelrole.do?id="+id, id, function(result) {
				if (result.success) {
		...
		function doDel(id){
			$.ajax({
				url: "${rootPath}menu/delrole.do?id=" + id, 
				async: true,
				success: function(o) {
