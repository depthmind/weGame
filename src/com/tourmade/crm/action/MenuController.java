...
public class MenuController extends BaseSimpleFormController {
	...
	public String roleEdit(Model model, String id) {
		if (null != id && !"".equals(id)) {
			...
			User user = service.getUserById(role_id);
	...
	@RequestMapping(value = "/queryUserById.do",produces="application/json;charset=utf-8")
	@ResponseBody
	public String queryUserByRoleId(Model model, String id) {
			int roleId = Integer.parseInt(id);
			QueryResult<User> userResult= service.getUserByRoleId(roleId);
			String userData=JSONUtilS.object2json(userResult);
			return userData;
	}
}
